...
- **Hero video** — the hero features the studio's RTÉ *Late Late Show* Antiques Special feature via YouTube's privacy-friendly `youtube-nocookie` embed.
- **Before & After** — the draggable before/after chair reveal now lives in its own section (`#beforeafter`) just below the hero.
- **Map** — the contact section has an OpenStreetMap embed pinned to the workshop (Eircode H91 W3V6, 53.30418, -8.93298) with a "Get directions" button that opens Google Maps directions. OpenStreetMap is used because Google's keyless `output=embed` iframe is now blocked by `X-Frame-Options`.
